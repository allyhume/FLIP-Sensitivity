...
metadata = nc_open("airt_daily_max_201501.nc")
...
metadata = nc_open("airt_daily_max_201001.nc")
lon = ncvar_get(metadata, "Longitude")
lat = ncvar_get(metadata, "Latitude")
...
for (x in seq(1,length(lon),sampleRate) ) {
	...
	if (!is.na(swrad_in[1])) { 
	   ...
	   colnames(df) <- c("lat","lon")
    	   write.table(df,        file=paste("lat_lon_2010_",x,"_",y,".csv",sep=""),row.names=FALSE)
	   write.table(sat_out,   file=paste("airt_2010_",x,"_",y,".csv",sep=""),   row.names=FALSE)
	   write.table(swrad_out, file=paste("swrad_2010_",x,"_",y,".csv",sep=""),  row.names=FALSE)
	 }
    }
}
