module weather_generator
  ...
  real,parameter :: lag_in_max_T    = 1.   ! Nos of hrs after Noon that max temp occurs.
...
contains
  ...
  subroutine daily_to_hourly( latitude_degs, day_number, days_in_year,  & ! intent(in)
    ...
    ! local variable..
    real :: declination, daylength, latitude, sunrise
    ...
    !
    !->CHECK WHETHER OR NOT WE ARE SUPPLYING DAILY-MEAN PRECIP, OR DAILY-TOTAL!
    !
    ppt_hr  = calc_ppt( ppt_day )
  ...
  subroutine sunrise_daylength( latitude , day_number , days_in_year , declination , sunrise , daylength )
    ...
    ! arguments..
    integer,intent(in) :: day_number
    real,   intent(in) :: days_in_year, latitude ! in radians
    real,  intent(out) :: daylength, declination, sunrise
...
!    integer :: hour
    ...
    if ( ( calcs .lt. -1.0 ) .or. ( calcs .gt. 1.0 ) ) then
      ! Days that have no sunlight at all...
      daylength = 0.0
    else
      daylength = 24.0 * acos( calcs ) / pi ! 23.94
    endif
  ...
  subroutine calc_sw( day_number, days_in_year , latitude , sw_day , declination , sunrise , daylength , sw_hr )
    ...
    ! arguments..
    integer,intent(in)  :: day_number
    real,   intent(in)  :: daylength, declination, latitude, &
                           sunrise, sw_day, days_in_year 
    real,   intent(out) :: sw_hr(24)

    ! local variables..
    integer :: hr,minhr,maxhr
    real    :: cos_solar_zenith_angle(24), day_angle, hour_angle

    ! initialise..
    sw_hr = 0.0
    cos_solar_zenith_angle = 0.
    ...
    do hr = 1 , 24
      ...
      if ( ( hr .gt. sunrise ) .and. ( hr .lt. sunrise+daylength ) ) then

         hour_angle = ( hr - 12 ) * 15. * ( pi / 180.0 )
         ...
         if ( cos_solar_zenith_angle(hr) .lt. 0.0 ) then
           ...
           print*,"At hour ",hr,"on day ",day_number,&
...
!           stop "Stopping!"
     ...
     minhr = ceiling(sunrise)
     maxhr = floor(sunrise+daylength)
     sw_hr( minhr:maxhr ) = max( ( ( 24.0 * sw_day ) / sum(cos_solar_zenith_angle( minhr:maxhr )) ) * &
                                 cos_solar_zenith_angle( minhr:maxhr ) , 0.0 )

     ! catch the error if cloudy day is sunnier that clear-sky day!
     if ( abs( (24.0*sw_day) - sum(sw_hr)) .gt. 1.0 ) then
         print*,"ERROR! More sunlight on cloudy day than in our clear-sky calculations!"
         print*,24.d0*sw_day,sum(sw_hr),latitude*(180.0/pi),day_number
!         stop "Stopping"
     endif
  ...
  subroutine calc_temp( tmax_day , tmin_day , tavg_day , sunrise , t_hr )
    ...
    ! arguments..
    real,intent(in)  :: tmax_day, tmin_day, tavg_day, sunrise
    real,intent(out) :: t_hr(24)
...
!    sat_at_midnight = 0.5 *(tavg_day+tmin_day)
    ...
    do hr = 1 , int(sunrise)
      ...
      t_hr( hr ) =  ( 1 / ( sunrise - 0. ) ) * &
               ( tmin_day * ( hr - 0. ) + sat_at_midnight * ( sunrise - hr ) )
    enddo

    hottest_time = 12 + lag_in_max_T
    ...
    sum_one = 0. ; sum_two = 0.
    do hr = index_hot_time+1 , 24
      sum_one = sum_one + ( hr - hottest_time )
      sum_two = sum_two + ( 24 - hr )
    enddo
    sat_at_midnight = ( 1 / sum_one) * ( &
            ( 24 - hottest_time ) * ( 24 * Tavg_day - sum(t_hr(1:index_hot_time) ) ) &
            - tmax_day * sum_two )

    ! Now calculate those post-PeakT hours..
    do hr = int(hottest_time)+1, 24
      ! straight line from Tmax downwards, in such a way as to maintain Tavg.
      t_hr( hr ) = ( 1 / ( 24 - hottest_time ) ) * &
               ( sat_at_midnight * ( hr - hottest_time ) + tmax_day * ( 24 - hr ) )
    enddo
  ...
  subroutine calc_rh( rh_max , rh_min , rh_avg , sunrise , rh_hr )
    ...
    ! We can get ahold of specific humidity, from which we should !
    ...
    ! arguments..
    real,intent(in)  :: rh_min, rh_max, rh_avg, sunrise
    real,intent(out) :: rh_hr(24)
...
!    rh_at_midnight = rh_max
    ...
    do hr = 1 , int(sunrise)
      ...
      rh_hr( hr ) =  ( 1 / ( sunrise - 0. ) ) * &
                        ( rh_max * ( hr - 0. ) + rh_at_midnight * ( sunrise - hr ) )
    ...
    rh_at_midnight = ( 1 / sum_one) * ( &
                   ( 24 - hottest_time ) * ( 24 * rh_avg - sum(rh_hr(1:index_hot_time) ) ) &
                   - rh_min * sum_two )

    ! Now calculate those post-PeakT hours..
    do hr = int(hottest_time)+1, 24
      ! straight line from RH_min upwards, in such a way as to maintain RH-avg.
      rh_hr( hr ) = ( 1 / ( 24 - hottest_time ) ) * &
                       ( rh_at_midnight * ( hr - hottest_time ) + rh_min * ( 24 - hr ) )
      ! sanity check however
      rh_hr( hr ) = min(1.0,max(0.0,rh_hr( hr )))
    enddo
