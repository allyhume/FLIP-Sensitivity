  ...
  subroutine weathergeneratorinterface( latitude   , nos_days   , & ! in
    ...
    real,dimension(nos_days),intent(in) :: latitude &     ! degrees
                                          ,time(nos_days) ! day of time series
    ...
    real,dimension(nos_days*24),intent(inout) :: sat_out   & ! hourly surface air temperature (oC)
    ...
    ! local variables..
    integer             :: i, day_number
    real, dimension(24) :: coa_hrly, ppt_hrly, rh_hrly, sat_hrly, sw_hrly, wind_hrly
    ...
    do i = 1 , nos_days
       ...
       ! Update the output variables..
         coa_out( (i-1)*24+1 : i*24 ) =  coa_hrly 
         ppt_out( (i-1)*24+1 : i*24 ) =  ppt_hrly 
          rh_out( (i-1)*24+1 : i*24 ) =   rh_hrly 
         sat_out( (i-1)*24+1 : i*24 ) =  sat_hrly 
       swrad_out( (i-1)*24+1 : i*24 ) =   sw_hrly  
        wind_out( (i-1)*24+1 : i*24 ) = wind_hrly
